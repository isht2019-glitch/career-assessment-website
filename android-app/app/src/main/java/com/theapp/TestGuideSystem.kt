...
class TestGuideSystem(private val context: Context, private val parentView: ViewGroup) {
    ...
    fun init() {
        ...
        // Start guide automatically and immediately for test page
        handler.postDelayed({
            startGuide()
        }, 100)
    }
    ...
    private fun showStep(step: Int) {
        showTyping(800) {
            ...
            when (step) {
                0 -> showExamIntroStep()
                1 -> showExamStructureStep()
            }
        }
    }
    
    private fun showExamIntroStep() {
        // Introduction text
        addTextView("I can see you are on this exam page. 📝", "#FF6B35", true)
        addTextView("Need not to worry! It is a simple exam. 😊", "#FFD700")
        
        // Continue button
        val continueBtn = createButton("Tell me more", "#4CAF50") {
            currentStep = 1
            showStep(1)
        }
    ...
    private fun showExamStructureStep() {
        showTyping(800) {
            speechContent.removeAllViews()
            
            addTextView("This exam is divided in two sections: 📋", "#FF6B35", true)
            addTextView("1️⃣ Personality Test: 30 questions (in 3 sets of 10)", "#FFFFFF")
            addTextView("2️⃣ Aptitude Test: 20 questions", "#FFFFFF")
            addTextView("You'll get personality insights after every 10 questions! 🧠", "#FFFFFF")
            addTextView("⏱️ Total Time: 50 minutes for all 50 questions", "#FFFFFF")
            addTextView("Result will be calculated on the basis of your performance! 🎯", "#FFD700", true)
            
            val startBtn = createButton("Got it! Let's start", "#4CAF50") {
                completeGuide()
            }
    ...
    private fun completeGuide() {
        ...
        prefs.edit()
            .putBoolean("test_guide_completed", true)
            .apply()
    }
    
    private fun shouldShowTestGuide(): Boolean {
        val prefs = context.getSharedPreferences("guide_prefs", Context.MODE_PRIVATE)
        return !prefs.getBoolean("test_guide_completed", false)
    }
